...
symlinkSync(join(root,'dist/AroliMonoNF-Regular.otf'),join(temp,'Aroli.otf'));
...
for(const extension of ['png','svg']) {
  const result=spawnSync('pango-view',['--no-display','--pixels','--font=Aroli Mono NF 26','--markup','--margin=40','--spacing=12','--background=#101111','--foreground=#C5C7C5',`--output=${join(root,`dist/specimen.${extension}`)}`,'--text',text],{env,encoding:'utf8'});
